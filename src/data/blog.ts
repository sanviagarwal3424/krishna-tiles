export interface BlogPost {
  slug: string;
  title: string;
  excerpt: string;
  category: string;
  date: string;
  readTime: string;
  content: string;
}

export const blogPosts: BlogPost[] = [
  {
    slug: "how-to-choose-tiles-for-bathroom",
    title: "How to Choose the Right Tiles for Your Bathroom",
    excerpt: "Choosing bathroom tiles can be overwhelming. Here's a simple guide to help you pick the right size, finish, and material for a stunning and practical bathroom.",
    category: "Buying Guide",
    date: "2026-03-15",
    readTime: "5 min read",
    content: `## How to Choose the Right Tiles for Your Bathroom

Selecting the right tiles for your bathroom is one of the most important decisions in any renovation. The wrong choice can make a bathroom feel cramped, dark, or high-maintenance.

### 1. Size Matters

- **Small bathrooms**: Use larger tiles (600x600mm or more) to create an illusion of space. Fewer grout lines = bigger look.
- **Large bathrooms**: You can experiment with smaller mosaic or hexagonal patterns.

### 2. Choose the Right Finish

- **Glossy**: Reflects light, looks luxurious. Best for walls.
- **Matte**: Hides water spots. Good for floors.
- **Anti-skid**: Essential for bathroom floors to prevent slipping.

### 3. Material Selection

- **Ceramic**: Affordable, lightweight, good for walls.
- **Porcelain/Vitrified**: Denser, more durable. Ideal for floors.

### 4. Color Psychology

- Light colors (white, beige, cream) make bathrooms feel larger.
- Dark tiles (charcoal, navy) create a spa-like premium feel.

### Pro Tip

Always buy 10% extra tiles to account for cuts and future replacements. Tile batches can vary in shade between production runs.

Visit our showroom at Upper Bazar, Ranchi — our experts will help you choose for free.`,
  },
  {
    slug: "best-tiles-for-living-room-2026",
    title: "Best Floor Tiles for Living Rooms in 2026",
    excerpt: "Marble-look tiles, large-format slabs, or wood planks? We break down the top flooring trends for Indian living rooms in 2026.",
    category: "Trends",
    date: "2026-03-05",
    readTime: "4 min read",
    content: `## Best Floor Tiles for Living Rooms in 2026

The living room is the first thing guests see. Your flooring sets the tone for your entire home's design.

### Trending in 2026

**1. Large Format Marble-Look Tiles**
Sizes like 800x1600mm or 1200x1200mm are dominating premium homes. Brands like Kajaria and Somany offer stunning marble looks at a fraction of natural marble's cost.

**2. Wood Plank Tiles**
The warmth of wood with the durability of porcelain. The 200x1200mm plank format creates a natural, inviting feel perfect for modern homes.

**3. Concrete/Cement Look**
Industrial-chic is in. Grey cement-look tiles give a sophisticated, minimalist aesthetic.

**4. Bookmatched Slabs**
Mirror-matched tiles create seamless, artwork-like floors. Best for larger spaces.

### What to Avoid

- Very light colors in high-traffic living rooms (shows dust easily)
- Highly polished floors near stairs or entrances (slipping hazard)

Browse our living room tile collection at Krishna Tiles, Upper Bazar, Ranchi.`,
  },
  {
    slug: "tile-maintenance-tips",
    title: "How to Keep Your Tiles Looking New for Years",
    excerpt: "Tiles are low-maintenance — but not no-maintenance. Follow these simple care tips to keep your floors and walls pristine for decades.",
    category: "Maintenance",
    date: "2026-02-20",
    readTime: "3 min read",
    content: `## How to Keep Your Tiles Looking New for Years

Good quality tiles can last 20-30 years. With the right care, they'll look new throughout.

### Daily Care

- Sweep or vacuum regularly to remove grit that scratches surfaces
- Mop with warm water and a mild floor cleaner
- Wipe spills immediately — especially on light-colored grout

### Grout Maintenance

Grout is the weakest point. To protect it:
- Seal grout lines every 1-2 years
- Use a grout pen to refresh color
- Clean with a baking soda paste for stubborn stains

### What NOT to Do

- Never use bleach on colored tiles
- Avoid acidic cleaners (vinegar, lemon) on marble-look tiles
- Don't use steel wool or abrasive pads

### Bathroom Tile Care

Run the exhaust fan or open windows after showers. Humidity is the #1 cause of grout mold. A weekly spritz of diluted vinegar solution on grout lines prevents buildup.

For tile-specific care advice, visit Krishna Tiles showroom in Ranchi.`,
  },
  {
    slug: "kitchen-backsplash-ideas",
    title: "10 Kitchen Backsplash Tile Ideas for Indian Homes",
    excerpt: "Your kitchen backsplash is a small canvas with big impact. Here are 10 tile ideas that work beautifully in Indian kitchens.",
    category: "Inspiration",
    date: "2026-02-10",
    readTime: "6 min read",
    content: `## 10 Kitchen Backsplash Tile Ideas for Indian Homes

The backsplash protects your walls from cooking splatter and defines your kitchen's personality. Here are our top picks:

1. **Classic White Subway Tiles** — Timeless, versatile, easy to clean
2. **Moroccan Patterned Tiles** — Adds personality and color
3. **Terracotta Rustic Tiles** — Warm, earthy, works with wooden cabinets
4. **Glossy Cream Metro Tiles** — Brightens darker kitchens
5. **Hexagonal Mosaic** — Modern and geometric
6. **Stone-Look Tiles** — Natural premium look
7. **Dark Grey Glossy** — Sleek, modern, hides stains well
8. **Hand-Painted Azulejo Style** — Artisanal, unique
9. **Large Format Single Color** — Minimal and clean
10. **Mixed Material Mosaic** — Mix of glass, ceramic, and stone

### Key Considerations for Indian Kitchens

- **Heat resistance**: Tiles near the stove must handle high heat
- **Grease resistance**: Glossy finish wipes clean easily
- **Size**: Smaller tiles (100x200, 100x300mm) work best for backsplashes

All these tile styles are available at Krishna Tiles, Upper Bazar, Ranchi.`,
  },
  {
    slug: "how-to-choose-the-right-tile",
    title: "How to Choose the Right Tile for Every Room",
    excerpt: "Floor, wall, kitchen, bathroom — each room demands a different tile. Here's a room-by-room guide to choosing the perfect tile for your Indian home.",
    category: "Buying Guide",
    date: "2026-04-12",
    readTime: "5 min read",
    content: `## How to Choose the Right Tile for Every Room

Not all tiles are made equal. What works in a bathroom will fail in a living room and vice versa. Here's a practical room-by-room breakdown.

### Living Room

- **Best material**: Glazed Vitrified Tiles (GVT) or polished porcelain
- **Ideal size**: 600x1200mm or 800x1600mm — fewer grout lines, larger visual space
- **Finish**: Polished or matt depending on furniture style
- **Budget**: ₹50-150/sq.ft for quality options

### Bathroom

- **Floors**: Anti-skid (PEI-3 minimum), matt finish, water-resistant
- **Walls**: Glossy ceramic for easy cleaning, moisture resistance
- **Size**: 300x600 for walls, 300x300 for floors (more grout = more grip)

### Kitchen

- **Backsplash**: Glossy, stain-resistant, heat-proof — subway or metro format
- **Floor**: Wear-rated, textured surface (spills happen)
- **Avoid**: Highly polished surfaces near cooking area

### Outdoor

- **Must-have**: R11 anti-slip rating, frost-proof, fade-resistant
- **Material**: Full-body porcelain or rustic vitrified
- **Thickness**: 10mm minimum, 20mm for driveways

### Pro Tips

- Always view tiles under your home's actual lighting before buying
- Order 10% extra for cuts, breakage, and future repairs
- Match grout colour to tile for seamless look, contrast grout for character

Visit Krishna Tiles at Upper Bazar, Ranchi for free expert guidance on room-specific tile selection.`,
  },
  {
    slug: "sanitaryware-trends-2026",
    title: "Sanitaryware Trends 2026: What's New in Indian Bathrooms",
    excerpt: "From wall-hung WCs to smart sensor faucets — here are the biggest bathroom trends shaping Indian homes in 2026.",
    category: "Trends",
    date: "2026-04-25",
    readTime: "4 min read",
    content: `## Sanitaryware Trends 2026: What's New in Indian Bathrooms

Indian bathrooms are evolving from utility spaces to wellness retreats. Here are the trends driving the change.

### 1. Wall-Hung Toilets

Floor space is premium in Indian bathrooms. Wall-hung WCs free up visual and physical space, making compact bathrooms feel larger. Brands like Jaquar and Cera now offer affordable wall-hung options.

### 2. Sensor-Driven Faucets

Touchless taps are no longer limited to airports. Home-grade sensor faucets from Jaquar and Kohler reduce water waste and improve hygiene — practical for Indian kitchens too.

### 3. Rain Shower Systems

Overhead rain showers are replacing hand-held setups in premium bathrooms. Thermostatic mixers maintain constant temperature regardless of water pressure changes.

### 4. Slim Cisterns and Concealed Tanks

Concealed flush systems with slim cisterns save 3-4 inches of wall depth. Worth planning during construction — retrofitting is harder.

### 5. Matte Black and Brushed Gold Finishes

Chrome is still the default, but matte black and brushed gold accents add a designer touch. Available across Jaquar Artize and Kohler Composed lines.

### Budget Planning

- Basic upgrade (WC + basin + faucet set): ₹15,000-30,000
- Mid-range (Jaquar/Cera full suite): ₹35,000-70,000
- Premium (Kohler/Artize): ₹1,00,000+

See working bathroom mock-ups at Krishna Tiles, Upper Bazar, Ranchi.`,
  },
  {
    slug: "porcelain-vs-ceramic-tiles",
    title: "Porcelain vs Ceramic Tiles: Which Should You Choose?",
    excerpt: "Confused between porcelain and ceramic? We break down the differences in durability, price, water absorption, and best use cases for Indian homes.",
    category: "Buying Guide",
    date: "2026-05-05",
    readTime: "4 min read",
    content: `## Porcelain vs Ceramic Tiles: Which Should You Choose?

This is the most common question we hear at Krishna Tiles. Here's a clear comparison.

### What's the Difference?

Both are made from clay fired in a kiln. The key difference is density:

- **Ceramic**: Fired at lower temperatures, more porous, lighter
- **Porcelain (Vitrified)**: Fired at higher temperatures, denser, less than 0.5% water absorption

### Durability Comparison

- **Porcelain**: PEI-4 to PEI-5 rated — handles heavy foot traffic, outdoor use
- **Ceramic**: PEI-1 to PEI-3 — suitable for walls and low-traffic floors

### Water Resistance

- **Porcelain**: Nearly waterproof — ideal for bathrooms, kitchens, outdoor
- **Ceramic**: Absorbs moisture over time — best for dry interior walls

### Price Comparison

- **Ceramic**: ₹25-60/sq.ft — budget-friendly
- **Porcelain**: ₹45-200/sq.ft — premium but longer-lasting

### When to Pick What

**Choose Ceramic for:**
- Bathroom and kitchen walls
- Interior accent walls
- Budget renovations
- Low-traffic areas

**Choose Porcelain for:**
- All floor applications
- Bathroom floors (anti-skid variants)
- Outdoor areas (balcony, parking, terrace)
- High-traffic commercial spaces

### Our Recommendation

For most Ranchi homes, we recommend porcelain for floors and ceramic for walls — this combination gives the best value.

Compare both side-by-side at our Upper Bazar showroom. Free consultation included.`,
  },
  {
    slug: "outdoor-tiles-buying-guide",
    title: "Outdoor Tiles Buying Guide: Balcony, Terrace & Parking",
    excerpt: "Outdoor tiles need to survive Ranchi's monsoons, summer heat, and heavy use. Here's how to choose the right ones for balconies, terraces, and parking areas.",
    category: "Buying Guide",
    date: "2026-05-18",
    readTime: "5 min read",
    content: `## Outdoor Tiles Buying Guide: Balcony, Terrace & Parking

Outdoor tiles face the harshest conditions — UV exposure, monsoon rain, temperature swings, and heavy loads. Choose wrong and you'll be re-tiling within 3-5 years.

### Key Requirements

- **Anti-slip**: R11 rating minimum (R12 for pool areas)
- **Water absorption**: Less than 0.5% (full-body porcelain)
- **Thickness**: 10mm for balconies, 20mm for parking and driveways
- **Frost-proof**: Essential for Ranchi winters

### Best Materials by Area

**Balcony & Terrace**
- Rustic porcelain in 600x600mm
- Stone-look or sandstone finishes
- Budget: ₹50-100/sq.ft

**Parking & Driveway**
- 20mm full-body porcelain (can support vehicle weight)
- Textured anti-skid surface mandatory
- Budget: ₹80-150/sq.ft

**Garden Pathways**
- Natural stone-look porcelain
- Earthy tones that blend with landscaping
- Budget: ₹45-80/sq.ft

### Common Mistakes

- Using interior polished tiles outdoors (slippery when wet)
- Skipping proper slope for water drainage
- Choosing dark tiles that absorb heat (burns bare feet in summer)
- Not using expansion joints (tiles crack in temperature swings)

### Brands We Recommend for Outdoor

- **Kajaria**: Heavy-duty rustic range rated for monsoons
- **RAK Ceramics**: R11 anti-slip stone-look porcelain
- **Johnson Tiles**: Endura wear-rated for parking areas
- **Nitco**: Naturoc stone-look for terraces

Visit Krishna Tiles, Upper Bazar, Ranchi for outdoor tile samples. We'll help you calculate quantities and choose the right slip rating.`,
  },
  {
    slug: "best-kajaria-tiles-ranchi-top-5",
    title: "Best Kajaria Tiles at Best Price in Ranchi — Top 5 Picks for 2026",
    excerpt: "Looking for Kajaria tiles in Ranchi at the best price? Here are our top 5 Kajaria tile picks for living rooms, bathrooms, kitchens and outdoors — available at Krishna Tiles, Upper Bazar.",
    category: "Buying Guide",
    date: "2026-05-30",
    readTime: "5 min read",
    content: `## Best Kajaria Tiles at Best Price in Ranchi — Top 5 Picks for 2026

Kajaria is India's largest tile manufacturer and one of the most trusted names in the industry. At Krishna Tiles, we're authorised Kajaria dealers in Ranchi — stocking their latest collections at competitive showroom prices with no hidden charges.

Whether you're building a new house, renovating a flat, or working on a commercial project, here are our top 5 Kajaria tiles that Ranchi customers love the most.

## 1. Kajaria Statuario Premium (Living Room)

![Kajaria Statuario Premium 800x1600mm polished vitrified tile](https://5.imimg.com/data5/SELLER/Default/2021/6/UJ/BJ/BI/38967491/kajaria-floor-tile-1000x1000.jpg)

- **Size**: 800 × 1600 mm
- **Material**: Glazed Porcelain
- **Finish**: High-gloss polished
- **Best for**: Living rooms, drawing rooms, lobbies

This is the tile that walks out of our showroom the fastest. The Statuario Premium gives you the look of imported Italian marble at a fraction of the cost. The large 800x1600 format means fewer grout lines and a seamless, luxurious floor. Perfect for Ranchi homes that want a premium feel without the premium marble price tag.

**Price range**: ₹85–120/sq.ft at Krishna Tiles

## 2. Kajaria Eternity Grande Marble (Kitchen/Feature Wall)

![Kajaria Eternity Grande Marble 1200x2400mm GVT mega slab](https://5.imimg.com/data5/SELLER/PDFImage/2026/1/579063587/EJ/QI/EX/262145004/1200x2400-mm-kajaria-eternity-the-ultima-tile-1000x1000.png)

- **Size**: 1200 × 2400 mm
- **Material**: GVT Mega Slab
- **Finish**: Polished bookmatched
- **Best for**: Kitchen islands, feature walls, large living rooms

The biggest tile in our Kajaria collection. At 1200x2400mm, a single slab covers almost 3 sq. metres. Ideal for creating dramatic bookmatched feature walls or seamless kitchen countertop backsplashes. This is the tile architects in Ranchi specifically request for premium projects.

**Price range**: ₹150–220/sq.ft at Krishna Tiles

## 3. Kajaria Carrara White (Bathroom)

![Kajaria Carrara White 600x1200mm glossy vitrified tile](https://5.imimg.com/data5/SELLER/Default/2023/6/319501583/HT/OX/BX/138810177/tt1-1000x1000.jpeg)

- **Size**: 600 × 1200 mm
- **Material**: Glazed Porcelain
- **Finish**: Glossy
- **Best for**: Bathroom walls, powder rooms, wash areas

Clean white marble look with subtle grey veining — the Carrara White is our best-selling bathroom tile. The 600x1200 format works perfectly for bathroom walls, creating an elegant spa-like feel. Pair it with a darker anti-skid floor tile for contrast.

**Price range**: ₹55–80/sq.ft at Krishna Tiles

## 4. Kajaria Herringbone Walnut (Kitchen Floor)

![Kajaria wood-look glazed vitrified plank tile in walnut finish](https://5.imimg.com/data5/SELLER/Default/2025/9/545151341/BO/BK/DK/48516260/roots-lend-wood-digital-glazed-vitrified-ceramic-tile-1000x1000.jpeg)

- **Size**: 100 × 600 mm
- **Material**: Wood-Look Porcelain
- **Finish**: Matte textured
- **Best for**: Kitchen floors, dining areas, bedrooms

Want the warmth of wooden flooring without the maintenance headaches? The Herringbone Walnut gives you a realistic wood grain in a durable porcelain body. Zero termite risk, zero polishing needed, and it handles water spills without warping. A favourite among young couples building their first home in Ranchi.

**Price range**: ₹65–90/sq.ft at Krishna Tiles

## 5. Kajaria Bluestone Outdoor (Balcony & Parking)

![Bluestone grey 600x600mm 20mm anti-slip outdoor paver tile](https://tilemall.com.au/cdn/shop/products/Paver-Blue-Stone20-600x600-Grip-Grey_9a151c7e-71f1-4648-97b1-6c3739b41af0.jpg)

- **Size**: 600 × 600 mm
- **Material**: 20mm Anti-Slip Porcelain
- **Finish**: Textured anti-skid
- **Best for**: Balconies, terraces, parking, driveways

Built for Ranchi's monsoons and summer heat. The 20mm thickness handles vehicle weight on driveways, while the R11 anti-slip rating keeps balconies and terraces safe in the rain. The natural bluestone look ages beautifully and hides dirt between cleanings.

**Price range**: ₹80–110/sq.ft at Krishna Tiles

## Why Buy Kajaria from Krishna Tiles, Ranchi?

- **Authorised dealer** — genuine Kajaria products with full manufacturer warranty
- **Best prices** — direct dealer pricing, no middleman markup
- **Free quantity calculation** — bring your floor plan, we'll calculate exactly how many tiles you need
- **Same-day delivery** — within Ranchi city limits
- **20+ years trusted** — serving Ranchi since 2004
- **Expert guidance** — our team helps you pick the right tile for your space, budget, and style

## Visit Our Showroom

**Krishna Tiles**
Lalji Hirji Road, Upper Bazar, Ranchi — 834001
Open Monday to Saturday, 10 AM – 8 PM

WhatsApp us your room measurements and we'll send you a free quote with the best Kajaria tile options for your budget.`,
  },
  {
    slug: "best-bathroom-tiles-ranchi-2026",
    title: "Best Bathroom Tiles in Ranchi 2026 — Top 5 Picks for Every Budget",
    excerpt: "Planning a bathroom renovation in Ranchi? Here are the 5 best bathroom tiles across Kajaria, Somany, Johnson and Orient Bell — available at Krishna Tiles, Upper Bazar.",
    category: "Buying Guide",
    date: "2026-06-03",
    readTime: "5 min read",
    content: `## Best Bathroom Tiles in Ranchi 2026 — Top 5 Picks for Every Budget

Your bathroom deserves the same design attention as your living room. The right tile can make a small bathroom feel spacious, a basic bathroom feel premium, and a wet room feel safe.

At Krishna Tiles, we've helped hundreds of Ranchi families choose bathroom tiles over the last 20 years. Here are our top 5 picks for 2026 — covering floors, walls, and accents.

## 1. Kajaria Anti-Skid Bathroom Floor Tile (Best for Safety)

![Kajaria anti-skid matte bathroom floor tile](https://5.imimg.com/data5/GLADMIN/Default/2023/4/303435621/PQ/KG/XF/18432698/kajaria-anti-skid-bathroom-tiles-1000x1000.jpg)

- **Size**: 300 × 300 mm / 600 × 600 mm
- **Material**: Matte Porcelain
- **Finish**: Anti-skid textured
- **Best for**: Bathroom floors, wet areas, shower zones

Safety first. Bathroom floors get wet every day, and a glossy tile here is an accident waiting to happen. Kajaria's anti-skid range gives you R10-R11 slip resistance without looking industrial. Available in stone-look, concrete-look, and neutral tones that pair well with any wall tile.

**Price range**: ₹40–70/sq.ft at Krishna Tiles

## 2. Somany Glossy Ceramic Wall Tile (Best for Bright Walls)

![Somany glossy ceramic wall tiles for bathrooms](https://5.imimg.com/data5/SELLER/Default/2022/10/RH/EE/KY/28875807/somany-ceramic-wall-tiles-1000x1000.jpg)

- **Size**: 300 × 600 mm
- **Material**: Glossy Ceramic
- **Finish**: High-gloss reflective
- **Best for**: Bathroom walls, powder rooms, half-height dado

Glossy wall tiles bounce light around the room, making even a 5x7 bathroom feel open. Somany's ceramic range comes in clean whites, soft beiges, and subtle patterns. The 300x600 format is the sweet spot for bathroom walls — large enough to look clean, small enough to handle curves and corners.

**Price range**: ₹30–55/sq.ft at Krishna Tiles

## 3. Johnson Decorative Bathroom Tile (Best for Character)

![Johnson Tiles decorative glossy bathroom tile](https://5.imimg.com/data5/SELLER/Default/2022/5/WL/BG/YQ/31488137/whatsapp-image-2022-05-24-at-4-05-17-pm-1000x1000.jpeg)

- **Size**: 300 × 600 mm / 300 × 450 mm
- **Material**: Glazed Ceramic
- **Finish**: Glossy with printed patterns
- **Best for**: Accent walls, feature bands, shower niches

If you want your bathroom to have personality, Johnson's decorative range delivers. Geometric patterns, floral motifs, and stone-effect prints add character without overwhelming the space. Use them as a single accent wall behind the basin or as a horizontal band at eye height — the rest of the walls in plain white.

**Price range**: ₹35–65/sq.ft at Krishna Tiles

## 4. Orient Bell Elevation Wall Tile (Best Budget Option)

![Orient Bell ceramic wall tiles for bathroom elevation](https://5.imimg.com/data5/SELLER/Default/2022/2/YO/RJ/WD/9489120/orient-bell-wall-tiles-1000x1000.jpg)

- **Size**: 300 × 450 mm / 300 × 600 mm
- **Material**: Ceramic
- **Finish**: Glossy / Satin
- **Best for**: Full bathroom walls, rental properties, budget renovations

Orient Bell gives you reliable quality at Ranchi's most affordable price point. Their bathroom wall tiles come in light colours — white, cream, ivory — that work with any floor tile. If you're renovating multiple bathrooms or working on a rental property where cost matters, this is the brand we recommend.

**Price range**: ₹25–45/sq.ft at Krishna Tiles

## 5. Kajaria Carrara White (Best Premium Pick)

![Kajaria Carrara White glossy marble-look tile for luxury bathrooms](https://5.imimg.com/data5/GLADMIN/Default/2023/4/303435623/CD/VS/RE/18432698/kajaria-anti-skid-bathroom-tiles-1000x1000.jpg)

- **Size**: 600 × 1200 mm
- **Material**: Glazed Porcelain
- **Finish**: Polished glossy
- **Best for**: Master bathroom walls, spa-style bathrooms, feature walls

For bathrooms that need to impress, the Carrara White is our top pick. The 600x1200 format with minimal grout lines creates a seamless marble wall effect. Pair it with a matte grey anti-skid floor tile for the classic luxury bathroom look. This is the tile interior designers in Ranchi request by name.

**Price range**: ₹55–80/sq.ft at Krishna Tiles

## Bathroom Tile Buying Checklist

- **Floor tiles**: Always anti-skid (R10 minimum). Matte or textured finish. Never use polished tiles on bathroom floors
- **Wall tiles**: Glossy or satin finish for easy cleaning and light reflection
- **Grout colour**: Use epoxy grout in bathrooms — it resists mold and doesn't absorb water like cement grout
- **Size guide**: 300x600mm for walls, 300x300mm for floors in standard bathrooms. Go larger (600x1200mm walls) in master bathrooms
- **Buy 10% extra**: Cuts, breakage, and future repairs. Tile batches vary in shade

## Why Buy Bathroom Tiles from Krishna Tiles?

- **50+ brands** — Kajaria, Somany, Johnson, Orient Bell, Cera, RAK all under one roof
- **Free design advice** — our team helps you pair wall and floor tiles that actually look good together
- **Quantity calculator** — bring your bathroom measurements, we'll calculate exact boxes needed
- **Same-day delivery** — within Ranchi city
- **20+ years in Ranchi** — we know what works in local homes

## Visit Our Showroom

**Krishna Tiles**
Lalji Hirji Road, Upper Bazar, Ranchi — 834001
Open Monday to Saturday, 10 AM – 8 PM

WhatsApp us your bathroom size and budget — we'll suggest the best wall + floor combination for you.`,
  },
];

export function getBlogPostBySlug(slug: string): BlogPost | undefined {
  return blogPosts.find((p) => p.slug === slug);
}
